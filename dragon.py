...
from Dragon import BaseBulkWalletChecker, BaseTopTraders, BaseTimestampTransactions, BaseScanAllTx
...
def base():
    walletCheck = BaseBulkWalletChecker()
    topTraders = BaseTopTraders()
    timestamp = BaseTimestampTransactions()
    scan = BaseScanAllTx()

    filesChoice, files = utils.searchForTxt(chain="Base")
    options, optionsChoice = utils.choices(chain="Base")
    ...
    while True:
        try:
            ...
            if optionsInput == 1:
                ...
                try:
                    while True:
                        ...
                        if fileSelectionOption > len(files):
                            ...
                            print(f"[🐲] Selected {files[fileSelectionOption - 1]}")
                            fileDirectory = f"Dragon/data/Base/{files[fileSelectionOption - 1]}"
                ...
                with open('Dragon/data/Base/TopTraders/tokens.txt', 'r') as fp:
                    contractAddresses = fp.read().splitlines()
                    if contractAddresses and contractAddresses != []:
                        print(f"[🐲] Loaded {len(contractAddresses)} contract addresses")
                    else:
                        print(f"[🐲] Error occurred, file may be empty. Go to the file here: Draon/data/Base/tokens.txt")
                        print(f"\n{optionsChoice}\n")
                        continue
                    data = topTraders.topTraderData(contractAddresses, threads, useProxies)
