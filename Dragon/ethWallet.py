...
class BaseBulkWalletChecker:
    ...
    def getTokenDistro(self, wallet: str, useProxies):
        url = f"https://gmgn.ai/defi/quotation/v1/rank/base/wallets/{wallet}/unique_token_7d?interval=30d"
        retries = 3
        tokenDistro = []
    ...
    def getWalletData(self, wallet: str, skipWallets: bool, useProxies):
        url = f"https://gmgn.ai/defi/quotation/v1/smartmoney/base/walletNew/{wallet}?period=7d"
        retries = 5
                    ...
                    if data['msg'] == "success":
                        ...
                        if skipWallets:
                            if 'buy_30d' in data and isinstance(data['buy_30d'], (int, float)) and data['buy_30d'] > 0:
                                self.totalGrabbed += 1
                                print(f"[🐲] Successfully grabbed data for {wallet} ({self.totalGrabbed})")#  and float(data['base_balance']) >= 1.0: (uncomment this to filter out insiders that cashed out already)
    ...
    def processWalletData(self, wallet, data, headers, useProxies):
        direct_link = f"https://gmgn.ai/base/address/{wallet}"
        ...
        winrate_30data = self.sendRequest.get(
            f"https://gmgn.ai/defi/quotation/v1/smartmoney/base/walletNew/{wallet}?period=30d", 
    ...
    def fetchWalletData(self, wallets, threads, skipWallets, useProxies):
        ...
        path = f"Dragon/data/Base/BulkWallet/wallets_{filename}"
