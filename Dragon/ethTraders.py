...
class EthTopTraders:
    ...
    def fetchTopTraders(self, contractAddress: str, useProxies):
        url = f"https://gmgn.ai/defi/quotation/v1/tokens/top_traders/base/{contractAddress}?orderby=profit&direction=desc"
        retries = 3
    ...
    def topTraderData(self, contractAddresses, threads, useProxies):
        ...
        with open(f'Dragon/data/Base/TopTraders/allTopAddresses_{identifier}.txt', 'w') as av:
            for address in self.allAddresses:
                av.write(f"{address}\n")

        if len(repeatedAddresses) != 0:
            with open(f'Dragon/data/Base/TopTraders/repeatedTopTraders_{identifier}.txt', 'w') as ra:
        ...
        with open(f'Dragon/data/Base/TopTraders/topTraders_{identifier}.json', 'w') as tt:
